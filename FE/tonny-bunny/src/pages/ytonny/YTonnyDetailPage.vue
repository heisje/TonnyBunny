...
export default {
	...
	methods: {
		...
		async acceptYTonnyApply(yTonnyApplySeq, helperSeq, unitPrice) {
			...
			await this.$store.dispatch("getAlert", clientAlert);
		}
		...
		// 			// 고객이 수락
		// 			this.stompClient.subscribe(`/sub/alert/accept/${seq}`, (res) => {
		// 				let request = JSON.parse(res.body);
		// 				this.acceptAlert = request;

		// 				if (this.isHelper) {
		// 					console.log("hihi 여기서 토스트가 뜨면 되지롱", this.acceptAlert);

		// 					let clientNickname = this.yTonnyDetail.client.nickName;
		// 					let title = this.yTonnyDetail.title;
		...
		// 			// 헬퍼가 신청
		// 			// this.stompClient.subscribe(`/sub/alert/apply/${seq}`, (res) => {
		// 			// 	let request = JSON.parse(res.body);
		// 			// 	this.applyAlert = request;
