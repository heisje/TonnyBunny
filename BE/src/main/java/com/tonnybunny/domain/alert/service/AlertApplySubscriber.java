...
@Service
public class AlertApplySubscriber implements MessageListener {
	...
	public void onMessage(Message message, byte[] pattern) {
		try {
			...
			String url;
			url = "/sub/alert/apply/" + alertLogRequestDto.getClientSeq();
			template.convertAndSend(url, alertLogRequestDto);

			url = "/sub/alert/apply/" + alertLogRequestDto.getHelperSeq();
			template.convertAndSend(url, alertLogRequestDto);
