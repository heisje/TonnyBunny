...
public class EmailService {
	...
	//인증번호 생성
	private final String authenticNumber = createKey();
	...
	public MimeMessage createMessage(String to) throws Exception {
		log.info("보내는 대상 : " + to);
		log.info("인증 번호 : " + authenticNumber);
	...
	public String sendSimpleMessage(String to) throws Exception {
		...
		try {
			redisUtill.setDataExpire(authenticNumber, to, 60 * 1L); // 유효시간 1분
			javaMailSender.send(message); // 메일 발송
		} catch (MailException es) {
			es.printStackTrace();
			throw new IllegalArgumentException();
		}
		return authenticNumber; // 메일로 보냈던 인증 코드를 서버로 리턴
	}
